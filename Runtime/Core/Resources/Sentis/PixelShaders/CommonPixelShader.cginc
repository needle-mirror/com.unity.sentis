...
#define DECLARE_TENSOR(X) Texture2D<float4> X##ptr; \
uint WidthShift##X, WidthMask##X; \
float4 SampleBlock##X(uint blockIndex) { \
return SampleBlock(X##ptr, WidthMask##X, WidthShift##X, blockIndex); \
} \
float4 SampleElements##X(uint4 blockIndex4, uint4 c4) { \
...
float4 SampleBlock##X(uint4 index) { \
