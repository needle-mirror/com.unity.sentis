...
namespace Unity.Sentis
{
    static class CommandBufferHelper
    {
        ...
        public static void UnrolledDispatchFast(this CommandBuffer cb, ComputeFunc fn, int numThread)
        {
            if (numThread == 0)
                return;
            ...
            int threadGroupY = ComputeHelper.IDivC(neededTG, (int)ComputeFunc.SafeDispatchLimit);
            int threadGroupX = ComputeHelper.IDivC(neededTG, threadGroupY);
            s_unrolledDispatchArgs[0] = threadGroupX;
            s_unrolledDispatchArgs[1] = numThread;
            cb.SetComputeIntParams(fn.shader, k_ID_unrolledDispatchArgs, s_unrolledDispatchArgs);

            // some GFX APIs / GPU hw/drivers have limitation of 65535 per dimension
            if (threadGroupX > ComputeFunc.SafeDispatchLimit || threadGroupY > ComputeFunc.SafeDispatchLimit || threadGroupZ > ComputeFunc.SafeDispatchLimit)
                D.LogWarning($"Exceeded safe compute dispatch group count limit per dimension [{threadGroupX}, {threadGroupY}, {threadGroupZ}] for {fn.kernelName}");
