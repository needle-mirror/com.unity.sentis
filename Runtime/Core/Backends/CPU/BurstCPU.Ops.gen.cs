...
public partial class CPUOps
{
    ...
    public virtual TensorFloat Min(TensorFloat[] tensors)
    {
        ...
        for (int t = 1; t < tensors.Length; t++)
        {
            ...
            job.ScheduleBatchXBO(Pin(A), Pin(B), Pin(curO, uploadCache: false), outputLength, 1024);
    ...
    public virtual TensorFloat ReduceMin(TensorFloat X, int[] axes, bool keepdim)
    {
        ...
        for (int i = 1; i < axes.Length; i++)
        {
            ...
            else
            {
                ...
                job.ScheduleBatchXO(Pin(X), Pin(Otmp, uploadCache: false), Otmp.shape.length, 1024);
    ...
    public virtual TensorFloat Softmax(TensorFloat X, int axis)
    {
        ...
        job.innerLength = X.shape.Strides(axis);
        job.reduceLength = X.shape[axis];
        job.ScheduleBatchXO(Pin(X), Pin(O, uploadCache: false), O.shape.length / job.reduceLength, 1024);
    ...
    public virtual Tensor Tril(Tensor X, int k)
    {
        ...
        job.diagonalK = k;
        job.ScheduleXO(Pin(X), Pin(O, uploadCache: false), X.shape.Length(0, -1), 32);
