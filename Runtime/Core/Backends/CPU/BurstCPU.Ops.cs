...
public partial class CPUOps
{
    ...
    /// <inheritdoc/>
    public virtual TensorFloat Conv(TensorFloat X, TensorFloat K, TensorFloat B, int groups, int[] stride, int[] pad, int[] dilation, Layers.FusableActivation fusedActivation)
    {
        if (X.shape.rank > 5)
            return ConvND(X, K, B, groups, stride, pad, dilation, fusedActivation);

        bool canFuseActivation = false;
        switch (fusedActivation)
        {
            case Layers.FusableActivation.None:
            case Layers.FusableActivation.Relu:
                canFuseActivation = true;
                break;
        }

        var Oshape = ShapeInference.Conv(X.shape, K.shape, B.shape, groups, stride, pad, dilation);
        var O = canFuseActivation ? NewOutputTensorFloat(Oshape) : NewTempTensorFloat(Oshape);
        if (O.shape.HasZeroDims())
            return O;

        var job = new ConvJob();
        int arrayLength = job.Prepare(X.shape, K.shape, O.shape, groups, stride, pad, dilation, fusedActivation);
        job.ScheduleXSBO(Pin(X), Pin(K), Pin(B), Pin(O, uploadCache: false), arrayLength, 1);

        if (!canFuseActivation)
            O = ApplyFusedActivation(O, fusedActivation);
    ...
    /// <inheritdoc/>
    public virtual TensorFloat Conv2DTrans(TensorFloat X, TensorFloat K, TensorFloat B, int[] stride, int[] pad, int[] outputAdjustment, Layers.FusableActivation fusedActivation)
    {
        ...
        var job = new Conv2DTransJob();
        job.inputHeight = X.shape[2];
        job.inputWidth = X.shape[3];
        job.outputHeight = O.shape[2];
        job.outputWidth = O.shape[3];
        job.kernelHeight = K.shape[2];
        job.kernelWidth = K.shape[3];
        job.strideHeight = stride[0];
        job.strideWidth = stride[1];
        job.padTop = pad[0];
        job.padLeft = pad[1];

        int batchCount = X.shape[0];
        int inputChannels = X.shape[1];
        int inputSize = job.inputHeight * job.inputWidth;
        int outputChannels = O.shape[1];
        int outputSize = job.outputHeight * job.outputWidth;
        int kernelSize = outputChannels * job.kernelHeight * job.kernelWidth;

        var columnBuffer = NewTempTensorFloat(new TensorShape(kernelSize, inputSize));
        var pinCB = Pin(columnBuffer, uploadCache: false);

        for (int batchIndex = 0; batchIndex < batchCount; batchIndex++)
        {
            ScheduleSGEMM(
                Pin(K), inputChannels, kernelSize,
                Pin(X), inputChannels, inputSize,
                pinCB, kernelSize, inputSize,
                transposeA: true, offsetY: batchIndex * inputChannels * inputSize);

            job.offsetO = batchIndex * outputChannels * outputSize;
            job.ScheduleXBO(pinCB, Pin(B), Pin(O, uploadCache: false), outputChannels, 1);
        }
    ...
    /// <inheritdoc/>
    public virtual TensorFloat Resize(TensorFloat X, float[] scale, Layers.InterpolationMode interpolationMode, Layers.NearestMode nearestMode = Layers.NearestMode.RoundPreferFloor, Layers.CoordTransformMode coordTransformMode = Layers.CoordTransformMode.HalfPixel)
    {
    ...
    public virtual TensorFloat DepthToSpace(TensorFloat X, int blocksize, Layers.DepthToSpaceMode mode)
    {
        ...
        job.ScheduleBatchXO(Pin(X), Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    public virtual void GlobalAverageVariancePool(TensorFloat O, TensorFloat X, int axis)
    {
        ...
        job.ScheduleXO(Pin(X), Pin(O, uploadCache: false), O.shape.length / 2, 1024);
    }
    ...
    public virtual TensorFloat ScaleBias(TensorFloat X, TensorFloat S, TensorFloat B)
    {
        ...
        job.ScheduleXSBO(Pin(X), Pin(S), Pin(B), Pin(O, uploadCache: false), X.shape[1], 1024);
    ...
    public virtual TensorFloat AxisNormalization(TensorFloat X, TensorFloat S, TensorFloat B, float epsilon)
    {
        ...
        job.ScheduleXSBWO(Pin(X), Pin(S), Pin(B), Pin(meanVariance), Pin(O, uploadCache: false), outerLength, 1024);
    ...
    public virtual Tensor Cast(Tensor X, DataType toType)
    {
        ...
        if (toType == DataType.Float)
        {
            ...
            job.ScheduleXO(Pin(X), Pin(O, uploadCache: false), O.shape.length, 1024);
        }
        else
        {
    ...
    public virtual TensorFloat PRelu(TensorFloat X, TensorFloat S)
    {
        ...
        job.ScheduleBatchXBO(Pin(X), Pin(S), Pin(O, uploadCache: false), outputLength, 1024);
    ...
    public virtual TensorFloat Einsum(string equation, params TensorFloat[] operands)
    {
        switch (operands.Length)
        {
            case 1:
            {
                ...
                job.ScheduleXO(Pin(operands[0]), Pin(O, uploadCache: false), outputShape.length, 1024);
                return O;
            }
            case 2:
            {
                ...
                job.ScheduleXBO(Pin(operands[0]), Pin(operands[1]), Pin(O, uploadCache: false), outputShape.length, 1024);
                return O;
            }
            default:
    ...
    public virtual Tensor Concat(Tensor[] tensors, int axis)
    {
        ...
        unsafe
        {
            ...
            job.count = O.shape.Length(0, axis);
            ...
            for (int i = 0; i < tensors.Length; ++i)
            {
                ...
                job.X = new CPUOps.ReadOnlyMemResource() { ptr = pinX.rawPtr };
                job.O = new CPUOps.ReadWriteMemResource() { ptr = (float*)pinO.rawPtr + offsetO };
                job.strideX = lengthX;
                job.length = lengthX;

                var jobFence = job.Schedule(JobHandle.CombineDependencies(pinX.fence, pinO.reuse));
                pinX.reuse = jobFence;
    ...
    /// <inheritdoc/>
    public virtual TensorFloat Pad(TensorFloat X, int[] pad, Layers.PadMode padMode, float constant)
    {
    ...
    public virtual TensorFloat Pow(TensorFloat A, TensorInt B)
    {
        ...
        job.ScheduleBatchXBO(Pin(A), Pin(B), Pin(O, uploadCache: false), outputLength, 1024);
        return O;
    }
    ...
    public virtual Tensor Where(TensorInt C, Tensor A, Tensor B)
    {
        ...
        job.ScheduleXSBO(Pin(C), Pin(A), Pin(B), Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    protected virtual Tensor CompressWithIndices(Tensor X, TensorInt indices, int numIndices, int axis)
    {
        ...
        job.ScheduleBatchXBO(Pin(X), Pin(indices), Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    public virtual Tensor GatherElements(Tensor X, TensorInt indices, int axis)
    {
        ...
        job.ScheduleXBO(Pin(X), Pin(indices), Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    public virtual Tensor ScatterElements(Tensor X, TensorInt indices, Tensor updates, int axis, Layers.ScatterReductionMode reduction)
    {
        ...
        job.ScheduleXBO(Pin(updates), Pin(indices), Pin(O, uploadCache: true), indices.shape.length,
    ...
    public virtual TensorFloat ScatterND(TensorFloat X, TensorInt indices, TensorFloat updates, Layers.ScatterReductionMode reduction)
    {
        ...
        job.ScheduleXSBO(Pin(X), Pin(indices), Pin(updates), Pin(O, uploadCache: true), updatesLength * indicesLength, 1024);
    ...
    public virtual Tensor[] TopK(TensorFloat X, int k, int axis, bool largest, bool sorted)
    {
        ...
        var pinX = Pin(X);
        var pinV = Pin(values, uploadCache: false);
        var pinI = Pin(indices, uploadCache: false);
        var job = new TopKJob();
    ...
    public virtual TensorFloat RoiAlign(TensorFloat X, TensorFloat Rois, TensorInt Indices, Layers.RoiPoolingMode mode, int outputHeight, int outputWidth, int samplingRatio, float spatialScale)
    {
        ...
        job.mode = mode;
        job.ScheduleBatchXSBO(Pin(X), Pin(Rois), Pin(Indices), Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    public virtual TensorFloat RandomNormal(TensorShape s, float mean, float scale, float? seed)
    {
        ...
        job.mean = mean;
        job.scale = scale;
        job.ScheduleO(Pin(O, uploadCache: false), O.shape.length, 1024);
    ...
    protected virtual void MemCopy(Tensor X, Tensor O, int length = -1, int offsetX = 0, int offsetO = 0)
    {
        ...
        job.offsetO = offsetO;
        job.length = length;
        job.ScheduleXO(Pin(X), Pin(O, uploadCache: false));
    }
    ...
    protected virtual void MemSet(Tensor O, int value, int length = -1, int offsetO = 0)
    {
        ...
        job.length = length;
        job.memValue = value;
        job.ScheduleO(Pin(O, uploadCache: false));
    }
    ...
    /// <param name="uploadCache">Whether to copy tensor data to CPU backend.</param>
