...
namespace Unity.Sentis.Layers
{
    ...
    class NonMaxSuppression : Layer
    {
        ...
        public override void Execute(ExecutionContext ctx)
        {
            ...
            // Allocate output selectedIndices tensor for worst case: the maximum number of output boxes has been selected for each class in each batch.
            // Note the format of data tuple in innermost dimension is (batchID, classID, boxID).
            var O = ctx.vars.AllocateTensorAndStore(index, new TensorShape(batchCount * classCount * maxOutputBoxesPerClass, 3), DataType.Int, ctx.backend.backendType) as TensorInt;
            ctx.backend.NonMaxSuppression(boxes, scores, O, maxOutputBoxesPerClass, iouThreshold, scoreThreshold, centerPointBox);
        }
