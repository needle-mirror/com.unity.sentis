...
public class TensorIndexing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var tensorB = new Tensor<float>(shape: shapeB);
        ...
        var tensorC = new Tensor<int>(shape: new TensorShape(2), srcData: arrayA, dataStartIndex: 4);
        Assert.AreEqual(5, tensorC[0]);
        Assert.AreEqual(6, tensorC[1]);

        // Tensors can also have 0-dim shape, in this case they are empty.
        var tensorE = new Tensor<float>(shape: new TensorShape(0, 4, 5));
